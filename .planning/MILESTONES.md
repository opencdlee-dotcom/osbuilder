...
5. **Common-person UX polish** (Phase 5) — 30-entry friendly-error dictionary with versioned format gate; 8 dev-team role briefs driving `[ROLE]` banners + `> In plain English` tutor lines; beginner-mode default with zero hits for 9 forbidden tech tokens; tech-writer step with humanizer integration + graceful degrade.
